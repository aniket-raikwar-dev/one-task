...
const sideItemsArray = [
  ...
  },
  {
    ...
    icon: `<svg
                  ...
                  fill="currentColor"
                >
                  <path d="M2 4C2 3.44772 2.44772 3 3 3H21C21.5523 3 22 3.44772 22 4V20C22 20.5523 21.5523 21 21 21H3C2.44772 21 2 20.5523 2 20V4ZM16 19H20V5H16V19ZM14 5H10V19H14V5ZM4 5V19H8V5H4Z"></path>
                </svg>`,
    path: "/boards",
  },
  {
...
const Sidebar = ({ collapsed }) => {
  const [sideItems, setSideItems] = useState(sideItemsArray);
  const location = useLocation();

  useEffect(() => {
    const path = location.pathname;
    const updateSideItems = sideItems.map((item) => {
      return {
        ...item,
        is_selected: item.path === path,
      };
    });
    setSideItems(updateSideItems);
  }, [location]);
      ...
      {collapsed ? (
        <div className="sidebar-mini">
          <div>
            ...
            <div className="side-item-mini-list">
              {sideItems.map((item) => (
                <Link
                  to={item.path}
                  key={item.id}
          ...
          <div className="side-item-mini-list">
            <Link to="/settings" className="side-mini-item">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
                fill="currentColor"
            ...
            <div className="side-item-list">
              {sideItems.map((item) => (
                <Link
                  ...
                  key={item.id}
                  className={
                    item.is_selected ? "side-item-selected" : "side-item"
                  }
                >
                  <div dangerouslySetInnerHTML={{ __html: item.icon }}></div>
                  <h3>{item.title}</h3>
